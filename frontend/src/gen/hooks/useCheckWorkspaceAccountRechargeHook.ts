...
import type {
	CheckWorkspaceAccountRecharge422Type,
...
type CheckWorkspaceAccountRechargeClient = typeof client<
	CheckWorkspaceAccountRechargeMutationResponseType,
	CheckWorkspaceAccountRecharge422Type,
	never
>;
 ...
 */
export function useCheckWorkspaceAccountRechargeHook(
	rechargeId: CheckWorkspaceAccountRechargePathParamsType["recharge_id"],
	options: {
		mutation?: UseMutationOptions<
			CheckWorkspaceAccountRecharge["response"],
			CheckWorkspaceAccountRecharge["error"],
			CheckWorkspaceAccountRecharge["request"]
		>;
	...
	return useMutation({
		mutationFn: async () => {
			const res = await client<
				CheckWorkspaceAccountRecharge["data"],
				CheckWorkspaceAccountRecharge["error"],
				CheckWorkspaceAccountRecharge["request"]
			>({
				method: "post",
