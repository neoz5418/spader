...
import type {
	CreateInstancePortForward422Type,
	CreateInstancePortForwardMutationRequestType,
	CreateInstancePortForwardMutationResponseType,
	CreateInstancePortForwardPathParamsType,
} from "../ts/CreateInstancePortForwardType";

type CreateInstancePortForwardClient = typeof client<
	CreateInstancePortForwardMutationResponseType,
	CreateInstancePortForward422Type,
	CreateInstancePortForwardMutationRequestType
>;
 ...
 */
export function useCreateInstancePortForwardHook(
	workspace: CreateInstancePortForwardPathParamsType["workspace"],
	zone: CreateInstancePortForwardPathParamsType["zone"],
	name: CreateInstancePortForwardPathParamsType["name"],
	options: {
		mutation?: UseMutationOptions<
			CreateInstancePortForward["response"],
			CreateInstancePortForward["error"],
			CreateInstancePortForward["request"]
		>;
	...
	const invalidationOnSuccess = useInvalidationForMutation(
		"useCreateInstancePortForwardHook",
	);
	return useMutation({
		mutationFn: async (data) => {
			const res = await client<
				CreateInstancePortForward["data"],
				CreateInstancePortForward["error"],
				CreateInstancePortForward["request"]
			>({
				method: "post",
