 ...
 */
export function useDeleteFileStorageHook(
	...
	options: {
		mutation?: UseMutationOptions<
			DeleteFileStorage["response"],
			DeleteFileStorage["error"],
	...
	const invalidationOnSuccess = useInvalidationForMutation(
		"useDeleteFileStorageHook",
	);
	return useMutation({
		mutationFn: async () => {
			const res = await client<
				DeleteFileStorage["data"],
				DeleteFileStorage["error"],
				DeleteFileStorage["request"]
			>({
				method: "delete",
