...
import type {
	ReplaceWorkspaceQuota422Type,
	ReplaceWorkspaceQuotaMutationRequestType,
	ReplaceWorkspaceQuotaMutationResponseType,
	ReplaceWorkspaceQuotaPathParamsType,
} from "../ts/ReplaceWorkspaceQuotaType";

type ReplaceWorkspaceQuotaClient = typeof client<
	ReplaceWorkspaceQuotaMutationResponseType,
	ReplaceWorkspaceQuota422Type,
	ReplaceWorkspaceQuotaMutationRequestType
>;
 ...
 */
export function useReplaceWorkspaceQuotaHook(
	workspace: ReplaceWorkspaceQuotaPathParamsType["workspace"],
	options: {
		mutation?: UseMutationOptions<
			ReplaceWorkspaceQuota["response"],
			ReplaceWorkspaceQuota["error"],
			ReplaceWorkspaceQuota["request"]
		>;
	...
	const invalidationOnSuccess = useInvalidationForMutation(
		"useReplaceWorkspaceQuotaHook",
	);
	return useMutation({
		mutationFn: async (data) => {
			const res = await client<
				ReplaceWorkspaceQuota["data"],
				ReplaceWorkspaceQuota["error"],
				ReplaceWorkspaceQuota["request"]
			>({
				method: "put",
