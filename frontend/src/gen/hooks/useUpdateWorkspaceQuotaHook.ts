...
type UpdateWorkspaceQuotaClient = typeof client<
	UpdateWorkspaceQuotaMutationResponseType,
	UpdateWorkspaceQuota422Type,
	UpdateWorkspaceQuotaMutationRequestType
>;
 ...
 */
export function useUpdateWorkspaceQuotaHook(
	workspace: UpdateWorkspaceQuotaPathParamsType["workspace"],
	options: {
		mutation?: UseMutationOptions<
			UpdateWorkspaceQuota["response"],
			UpdateWorkspaceQuota["error"],
			UpdateWorkspaceQuota["request"]
		>;
	...
	const invalidationOnSuccess = useInvalidationForMutation(
		"useUpdateWorkspaceQuotaHook",
	);
	return useMutation({
		mutationFn: async (data) => {
			const res = await client<
				UpdateWorkspaceQuota["data"],
				UpdateWorkspaceQuota["error"],
				UpdateWorkspaceQuota["request"]
			>({
				method: "patch",
