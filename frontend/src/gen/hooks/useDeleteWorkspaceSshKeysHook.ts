...
import type {
	DeleteWorkspaceSshKeys422Type,
	DeleteWorkspaceSshKeysMutationResponseType,
	DeleteWorkspaceSshKeysPathParamsType,
} from "../ts/DeleteWorkspaceSshKeysType";

type DeleteWorkspaceSshKeysClient = typeof client<
	DeleteWorkspaceSshKeysMutationResponseType,
	DeleteWorkspaceSshKeys422Type,
	never
>;
 ...
 */
export function useDeleteWorkspaceSshKeysHook(
	workspace: DeleteWorkspaceSshKeysPathParamsType["workspace"],
	name: DeleteWorkspaceSshKeysPathParamsType["name"],
	options: {
		mutation?: UseMutationOptions<
			DeleteWorkspaceSshKeys["response"],
			DeleteWorkspaceSshKeys["error"],
			DeleteWorkspaceSshKeys["request"]
		>;
	...
	const invalidationOnSuccess = useInvalidationForMutation(
		"useDeleteWorkspaceSshKeysHook",
	);
	return useMutation({
		mutationFn: async () => {
			const res = await client<
				DeleteWorkspaceSshKeys["data"],
				DeleteWorkspaceSshKeys["error"],
				DeleteWorkspaceSshKeys["request"]
			>({
				method: "delete",
