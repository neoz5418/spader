...
import type {
	ReplaceUserQuota422Type,
	ReplaceUserQuotaMutationResponseType,
	ReplaceUserQuotaPathParamsType,
} from "../ts/ReplaceUserQuotaType";

type ReplaceUserQuotaClient = typeof client<
	ReplaceUserQuotaMutationResponseType,
	ReplaceUserQuota422Type,
	never
>;
 ...
 */
export function useReplaceUserQuotaHook(
	username: ReplaceUserQuotaPathParamsType["username"],
	options: {
		mutation?: UseMutationOptions<
			ReplaceUserQuota["response"],
			ReplaceUserQuota["error"],
			ReplaceUserQuota["request"]
		>;
	...
	const invalidationOnSuccess = useInvalidationForMutation(
		"useReplaceUserQuotaHook",
	);
	return useMutation({
		mutationFn: async () => {
			const res = await client<
				ReplaceUserQuota["data"],
				ReplaceUserQuota["error"],
				ReplaceUserQuota["request"]
			>({
				method: "put",
