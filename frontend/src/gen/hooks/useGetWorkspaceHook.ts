...
import type {
	GetWorkspace422Type,
	GetWorkspacePathParamsType,
	GetWorkspaceQueryResponseType,
} from "../ts/GetWorkspaceType";

type GetWorkspaceClient = typeof client<
	GetWorkspaceQueryResponseType,
	GetWorkspace422Type,
	never
>;
...
export const getWorkspaceQueryKey = (
	workspace: GetWorkspacePathParamsType["workspace"],
) =>
	[
		"v5",
		{
...
export function getWorkspaceQueryOptions(
	workspace: GetWorkspacePathParamsType["workspace"],
	options: GetWorkspace["client"]["parameters"] = {},
) {
 ...
 */
export function useGetWorkspaceHook<
	TData = GetWorkspace["response"],
	TQueryData = GetWorkspace["response"],
	TQueryKey extends QueryKey = GetWorkspaceQueryKey,
>(
	workspace: GetWorkspacePathParamsType["workspace"],
	options: {
		query?: Partial<
...
export const getWorkspaceSuspenseQueryKey = (
	workspace: GetWorkspacePathParamsType["workspace"],
) =>
	[
		"v5",
...
export type GetWorkspaceSuspenseQueryKey = ReturnType<
	typeof getWorkspaceSuspenseQueryKey
>;
export function getWorkspaceSuspenseQueryOptions(
	workspace: GetWorkspacePathParamsType["workspace"],
	options: GetWorkspace["client"]["parameters"] = {},
) {
 ...
 * @link /apis/workspace/v1/workspaces/:workspace
 */
export function useGetWorkspaceHookSuspense<
	TData = GetWorkspace["response"],
	TQueryKey extends QueryKey = GetWorkspaceSuspenseQueryKey,
>(
	workspace: GetWorkspacePathParamsType["workspace"],
	options: {
		query?: Partial<
	...
	const { query: queryOptions, client: clientOptions = {} } = options ?? {};
	const queryKey =
		queryOptions?.queryKey ?? getWorkspaceSuspenseQueryKey(workspace);
	const query = useSuspenseQuery({
		...(getWorkspaceSuspenseQueryOptions(
			workspace,
			clientOptions,
